...
The two disagree on 2,835 pads: on every one, the structural offset resolves and the last
4 bytes are 0, so each of them currently loses its net. The trailing string is empty on
5,960 pads; the other 43 are all in `Y93-PD1818 YiDianTong`, with values like `OL` and
`375`.
...
| `===原理图` | 6 | one line: a PDF file name (6 of 6) |
| `===` (no name) | 4 | whitespace-separated fields: 3 on 5,637 of 5,640 lines |
| `===阻值表` | 3 | `NETNAME=value` lines, e.g. `PP3V0_TRISTAR=379` |
| `===阻值图`, `===电压`, `===RFFE` | 1 each | `NETNAME=value` (e.g. `I2C1_AP_TO_MIC2_SCL=395`); `NETNAME=voltage` (e.g. `PP1V8=1.8V`); one line starting `[{"BusName"` |
...
a single line. `parseXzzTailAnnotations()`
